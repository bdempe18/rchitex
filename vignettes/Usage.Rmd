...
At its barebones, `describe` applies a list of summary statistics to all the numeric columns of a data frame or tibble^[For the sake of brevity, I will just write drata frame instead of "data frame and tibble.". All functionality for rchitex works for both structures.] The default functions (in order) are
...
The default setting of `describe` output the summary table as text output, perfect for early exploratory work. Simplying providing a path to the argument `path` will simultaneously output a text table to the console and write a tex file to the provided path. Setting `silent = TRUE` will supress the text output

 
``` {r textTables}
state_df <- tibble::as_tibble(datasets::state.x77)
describe(state_df)
# describe(state_df, path = getwd(), silent = TRUE)

```
The default functions can be overwritten to either change the order of the operations,
rename the operations, or apply additional (including self-defined) functions. The `summary` argument can be fed a named vector or a list of function names and the summary function. Note that `describe` supports user-defined functions as long as the function maps a vector of data to a single value, $f: \mathbb{R}^d \mapsto \mathbb{R}$.
