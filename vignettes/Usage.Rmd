...
#build(mod, mod2, mod, mod2, mod, mod2, grouped_label = list('first'=c(1,3), "Bonus"=3, "Extra"=5))

#build(mod, mod2, md = 'html')
build(mod, mod2, mod, mod2, mod, mod2, grouped_label = list('first'=c(1,3), "Bonus"=4, "Extra"=5), md='html')
```

### Variable Names

Independent variable names can be customized by passing a list associating variable names with the desired output format. Names exluded from the list will be excluded from the table.

Dependent variable names can be customized by passing a character vector of desired model labels. By default, the models are numbered (1), (2), (3) as shown in the previous chunk. Passing a NA value to the `dep_names` parameter result in the dependent variable name of the model being used. 
...
``` {r titleNotes, results='asis'}
    ...
    build(mod, mod2, indep_names=id_names, md='html', sig=sig,
        title=title, note=note, dep_names=dep_names)
```  
...
Occasionally you may want to specify that specific columns belong together (e.g. columns 1 through 3 are OLS while column 4 is a logit). Grouped labels can be activated by passing a list of labels and their associated columns to the `grouped_label parameter. The value of each lable should be a vector of length 1 or 2 signaling the first and last column in the grouping. For groups of size 1, you may pass a single number specifying the intended column.

``` {r grouping, results='asis'}
groups <- list("OLS" = c(1,2), "Repeat" = 3, "Extra" = 4)

build(mod, mod2, mod, mod2, indep_names=id_names, md='html', sig=sig,
      title=title, note=note, stats=stats, pre_stats=pre_stats,
      grouped_label=groups)
```
...
appendize(tex1, tex2, raw_gg, wrapped_gg, dir=fp)
```
