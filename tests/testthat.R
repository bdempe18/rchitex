...
test_that("number of obs correctly calculated for RSE and adj.SE") {
	...
	date3 <- data.frame(y = sample(c(0,1), 20, replace=TRUE),
			    x = rnorm(20, 15, 3))
...
test_that("primative functions of RSE and adj.se object do not throw error") {
	data1 <- data.frame(y = rnorm(50, 20, 5), x = rnorm(50, 10, 7))
	func <- function(x) runif(1)
	se <- rep(5, nrow(data1))
	mod1 <- lm(y ~ x, data=data1)
	mod2 <- glm(y ~ x, data=data1, family="binomial")
